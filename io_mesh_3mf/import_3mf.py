...
class Import3MF(bpy.types.Operator, bpy_extras.io_utils.ImportHelper):
    ...
    def __init__(self):
        ...
        # Store the base color of objects generated by Bambu Studio
        self.bambu_object_base_color = {}
    ...
    def execute(self, context):
        ...
        for path in paths:
            ...
            # Read bambu studio materials
            for file in files_by_content_type.get('', []):
                if file.name == 'Metadata/project_settings.config':
                    data = json.load(file)
                    index = 0
                    for color in data['filament_colour']:
                        blender_color = self.convert_color(color)
                        self.resource_materials[bambu_studio_colors[index]] = {}
                        self.resource_materials[bambu_studio_colors[index]][0] = ResourceMaterial(name=color, color=blender_color)
                        index = index + 1
                if file.name == 'Metadata/model_settings.config':
                    document = xml.etree.ElementTree.ElementTree(file=file)
                    for obj in document.findall('object'):
                        obj_id = obj.get('id')
                        # Find the metadata element with key='extruder'
                        extruder_value = '1'
                        for metadata in obj.findall('metadata'):
                            if metadata.get('key') == 'extruder':
                                extruder_value = metadata.get('value')
                                break
                        self.bambu_object_base_color[obj_id] = bambu_studio_colors[int(extruder_value) - 1]
    ...
    def convert_color(self, color_string):
        # Parse the color. It's a hexadecimal number indicating RGB or RGBA.
            color = color_string.lstrip("#")  # Should start with a #. We'll be lenient if it's not.
            ...
            if len(color) == 6:  # RGB format.
                return (b3, b2, b1, 1.0)  # b1, b2 and b3 are B, G, R respectively. b4 is always 0.
            else:  # RGBA format, or invalid.
                return (b4, b3, b2, b1)  # b1, b2, b3 and b4 are A, B, G, R respectively.
    ...
    def read_triangles(self, object_node, default_material, material_pid):
        ...
        for triangle in object_node.iterfind("./3mf:mesh/3mf:triangles/3mf:triangle", MODEL_NAMESPACES):
            ...
            try:
                ...
                try:
                    material = self.resource_materials[pid][0]
